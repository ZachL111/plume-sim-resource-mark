...
`plume-sim-resource-mark` is centered on simulations work in C#. The technical target is: Create a C# reference implementation for resource workflows, centered on event replay, fixture event logs, and golden state snapshots.
...
The score is deterministic. The current threshold is 152. Risk has a penalty of 5, latency has a penalty of 2, and weight contributes a bonus of 2.
